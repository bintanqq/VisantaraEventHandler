...
public class ForgeMenuListener implements Listener {
    ...
    public ForgeMenuListener(VisantaraEventHandler plugin, ForgeMenuConfig menuConfig) {
        this.plugin = plugin;
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (event.getClickedBlock() == null)
            return;
        if (!menuConfig.isTriggerBlock(event.getClickedBlock().getLocation()))
            return;
        ...
        plugin.getServer().getScheduler().runTask(plugin, () -> openStation(player, "material"));
    }
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        if (event.getRawSlot() != menuConfig.getSwitchSlot()) return;
        ...
        if (displayName.contains(menuConfig.getMaterialTrigger().toUpperCase())) {
            targetType = "material";
        } else if (displayName.contains(menuConfig.getNatureTrigger().toUpperCase())) {
            targetType = "nature";
        }

        if (targetType != null) {
            event.setCancelled(true);
            final String finalTarget = targetType;
            plugin.getServer().getScheduler().runTask(plugin, () -> {
                player.closeInventory();
                openStation(player, finalTarget);
            });
        }
    }

    private void openStation(Player player, String type) {
        ...
        String stationId = type.equals("nature") ? rankData.natureStation() : rankData.materialStation();
        if (stationId == null || stationId.isEmpty()) {
            plugin.getLogger().severe("[ForgeMenu] Station ID belum diatur untuk rank: " + rankName + " (tipe: " + type + ")");
            return;
        }

        // Jalankan perintah dari console untuk membuka menu MMOItems
        plugin.getServer().dispatchCommand(plugin.getServer().getConsoleSender(),
                "mi stations open " + stationId + " " + player.getName());
    }
